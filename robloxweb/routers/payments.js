...
router.post("/gamemoney-notification", async (req, res) => {
	console.log("Вход в уведы: " + JSON.stringify(req.body));
	let info = config.reserved[req.body.project_invoice] ? { ...config.reserved[req.body.project_invoice] } : undefined;
	if(!info) return res.status(200).json({ ok: true });
	...
	await axios.post(paymentHook, {
		content: `@everyone поступил заказ 🔔
Метод: Трансфер 🔁
💸 Оплачено: ${req.body.amount} RUB
🪙 Робуксов к получению ${info.sum} R$
👤 Никнейм: ${info.username}
🕔 Время: ${date.toLocaleString()}
🔗 Ссылка: https://www.roblox.com/games/${info.gameId}/
🆔 Заказа: ${req.body.project_invoice}`
	}).catch(e => {});
	...
	if(!inited.ok) return [
		config.release(req.body.project_invoice),
		autoError(req.body.project_invoice, date, "Невалид куки")
	];

	let passes = await RobloxParser.fetchPasses(info.gameId);
	if(!passes.ok || passes.data.length < 1) return [
		config.release(req.body.project_invoice),
		autoError(req.body.project_invoice, date, "Не удалось спарсить Gamepass'ы")
	];
	let pass = passes.data.filter(p => p.price == info.sum)?.[0];
	if(!pass) return [
		config.release(req.body.project_invoice),
		autoError(req.body.project_invoice, date, "Нет подходящего Gamepass'а")
	];

	let csrf = await user.getCsrfToken();
	if(!csrf.ok) return [
		config.release(req.body.project_invoice),
		autoError(req.body.project_invoice, date, "Не удалось получить CSRF-токен")
	];
	let userId = `${info.userId}`;
	let result = await user.buyPass(info.userId, csrf.token, pass);
	if(!result?.ok) return [
		config.release(req.body.project_invoice),
		autoError(req.body.project_invoice, date, "Не удалось купить Gamepass")
	];
	await user.revokePass(csrf.token, pass);
	if(config.stats[info.cookie]) config.stats[info.cookie].balance -= info.sum;
	config.release(req.body.project_invoice);
	updateOrder(req.body.project_invoice);
	await axios.post(autoHook, {
		content: `📤 ${Math.floor(info.sum / 100 * 70)}/${info.sum} R$ игроку ${info.username}
🕔 Время: ${date.toLocaleString()}
🆔 Заказа: ${req.body.project_invoice}`
	}).catch(e => {});
	console.log("Выдача закончена" + JSON.stringify(req.body));
});
